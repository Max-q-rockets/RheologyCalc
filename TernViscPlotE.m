function Output = TernViscPlotE(vfrac, MC, FM, x, y)
sl = vfrac;
if (0>MC || MC>1) || (0>FM || FM > 1)
    error('out of range');
end
FC = FM*MC;
Crowd = @(ratio, vf, vc) (1-(1-ratio)^3.61)*((1-(vf/(vf+vc)))*vf/(vf+vc));
...
for ci=1:length(C)
    ...
    for mi=1:(length(M)-ci-1)
        ...
        vf = sl-vm-vc;
        liq=1-sl;
        CrowdMC = Crowd(MC, vm, vc);
        CrowdFM = Crowd(FM, vf, vm);
        CrowdFC = Crowd(FC, vf, vc);
        vf2 = vf*(1-CrowdFC)*(1-CrowdFM);
        vc2 = vc*(1-CrowdFC)*(1-CrowdMC);
        vm2 = vm*(1-CrowdFM)*(1-CrowdMC);
        v4 = sl-vc2-vm2-vf2;
        fracf = vf2/(liq+vf2);
        fracm = vm2/(liq+vf2+vm2);
        fracc = vc2/(liq+vf2+vm2+vc2);
        frac4 = v4;
        Output(ci, mi) = H(fracf)*H(fracm)*H(fracc)*H(frac4);            
    end
end
