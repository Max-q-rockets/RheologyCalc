function Output = TernViscPlotV1(amounts, sizes, ind1, ind2, ind3, x, y) %sl = solids loading by volume (between 0 and 1), MC = ratio of medium/coarse sizes, FM = ratio of fine/medium sizes, x&y are optional if you want to calculate a specific point
  warning('off','all');
C = [0:0.001:1];
M = [0:0.001:1];
Output = 1./zeros(length(C), length(M));
sum1 = amounts(ind1)+amounts(ind2)+amounts(ind3);
for ci=1:length(C)
    c = C(ci);
    for mi=1:(length(M)-ci-1)
        m = M(mi);
        amounts1 = amounts;
        amounts1(ind1) = sum1*c;
        amounts1(ind2) = sum1*m;
        amounts1(ind3) = sum1*(1-(c+m));
